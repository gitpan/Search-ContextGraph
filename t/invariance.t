...
# invariance.t - checks to make sure that search results are not affected by 
# the order docs are added, or any zero-net-change add/delete operations
# that take place
...
my $count = $g->doc_count();
is( $count, 34, "Got correct document count" );
my ( $docs, $words ) = $g->search( 'pig' );

is( scalar keys %{$docs}, 2, "Correct number of results" );
ok( exists $docs->{'doc-4'},  "Result is sane" );
ok( exists $docs->{'doc-25'},  "Result is sane" );
ok( abs( $docs->{'doc-4'} - 0.894 ) < .01, , "Result is correct" );
ok( abs( $docs->{'doc-25'} - 1.425 ) < .01,  "Result is correct" );



# Search results should not change if the search is repeated
( $docs, $words ) = $g->search( 'pig' );

is( scalar keys %{$docs}, 2, "Correct number of results" );
ok( exists $docs->{'doc-4'},  "Doc A still present in result set" );
ok( exists $docs->{'doc-25'},  "Doc B still present in result set" );
ok( abs( $docs->{'doc-4'} - 0.894 ) < .01, , "Relevance for A is unchanged" );
ok( abs( $docs->{'doc-25'} - 1.425 ) < .01,  "Relevance for B is unchanged" );


# Check to make sure number of neighbors 

is( $g->degree( 'D:doc-4'), 58, "Doc node has correct degree" );
is( $g->degree( 'T:pig'), 2, "Term node has correct degree" );
...
ok( !exists $docs->{'doc-4'},  "Deleted doc missing from result set" );
ok( exists $docs->{'doc-25'},  "Undeleted doc still present in result set" );
ok( abs( $docs->{'doc-25'} - 1.425 ) > .01,  "Result score changed" );


$g->add( 'doc-4', $docs{'doc-4'} );
...
is( $g->degree( 'D:doc-4'), 58, "Re-added node has correct degree" );
is( $g->degree( 'T:pig'), 2, "Term node has correct updated degree" );
...
ok( exists $docs->{'doc-4'},  "Result is sane" );
ok( exists $docs->{'doc-25'},  "Result is sane" );
is( substr( $docs->{'doc-4'}, 0,5) , 0.894 , "Result is correct" );
is( substr(  $docs->{'doc-25'}, 0, 5 ) , 1.425,  "Result is correct" );
...
doc-15,curri,love children,results taste,won tons,ton,truffl,children,peopl,tortellini,children of chinese won tons,children of chinese won,taste like the love,cook,wong,mr,won,italian tortellini,taste like the love children,cooks from nepal,nepal,chinese won,results taste like the love,himalaya,love children of chinese won,tast,chinese won tons,meat,love,mr. wong,result
...
doc-32,many new yorkers,overr,restaurateur behind bond street,few months,street,month,zilliax,truffl,soho restaurant,brooklyn,jonathan,restaurateur,ami,student,graduate student,good news,graduat,new,yorker,brooklyn heights,soho restaurant last year,restaurant last year,height,bond,jonathan morr,last year,soho,morr,restaurateur behind bond,bond street,restaur,year,new yorkers,amy zilliax
