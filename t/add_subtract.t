...
# add/subtract - test suite to see if documents are properly added and 
...
while ( <DATA> ) {
	...
	$g->add_document( $title, \@words);
	$doc_4 = [$title, \@words ] if /doc-4/;
}

my $count = $g->doc_count();
is( $count, 34, "Got correct document count" );
is( $g->doc_count( 'dish' ), 2, "Correct term span for 'dish'" );
is( $g->doc_count( 'pig' ), 2, "Correct term span for 'pig'" );
is( $g->doc_count( 'omelet' ), 1, "Correct term span for 'omelet'" );
is( $g->doc_count( 'ounc' ), 3, "Correct term span for 'ounc'" );

is( $g->word_count( 'pig' ), 2, "Correct total word count for 'pig'" );

is( $g->term_count(), 876, "Total term count is correct" );

$g->delete( 'doc-4' );
is( $g->doc_count(), 33, "Document count updates correctly on delete" );
is( $g->term_count(), 826, "Total term count updates correctly on delete" );
is ( $g->doc_count( 'pig' ), 1, "Term span went down for 'pig'" );
is ( $g->word_count( 'pig' ), 1, "Word count went down for 'pig'" );


my $new_doc = { 'pig' => 4, 'ounc' => 1 };
ok( $g->add_document( 'doc-x', $new_doc ), "Added new doc" );
is( $g->term_count(), 826, "Overall term count stayed constant" );

is ( $g->doc_count( 'pig' ), 2, "Term span went up for 'pig'" );
is ( $g->word_count( 'pig' ), 5, "Word count went up correctly for 'pig'" );

ok( $g->delete( 'doc-x' ), "Deleted new doc" );
is( $g->term_count(), 826, "Overall term count stayed constant" );

ok( $g->add_document( 'doc-4', $doc_4->[1] ), "Re-added document 4" );
is( $g->term_count(), 876, "Overall term count returned to original value" );

for ( 1..34 ) { $g->delete( "doc-$_" ) }

is( $g->doc_count(), 0, "Document count went to zero" );
is( $g->term_count(), 0, "Term count went to zero" );
